...
{
  int arr[] = {pm25, pm10, temperature, pressure, humidity, noise, gasResistance};
  for (int i = 0; i < 3; i++)
  {
    if (arr[i] != -1)
    {
...
void SensorData::printMeasurements()
{
  char payloadStr[250];
  sprintf(payloadStr, "temp: %d, humidity: %d, pressure: %d", temperature, humidity, pressure);
