...
void discoverAndSetStatus() {
  ...
  if (validData) {
    ...
    int count = helpers::Util::splitCommand(&data, ':', readFields, 4);
    if (count != 4) {
      validData = false;
      ...
      #endif
      channelName = readFields[0];
      token = readFields[1];

      ssid = readFields[2];
      password = readFields[3];
      validData = true;
    }
  }
...
void handleRootGet() {
  ...
  String output = "<!DOCTYPE html> \
    ...
    <div><input type='text' name='channelName' /><br/><br/></div> \
    <div>Token:</div> \
    <div><input type='text' name='token' /><br/><br/></div> \
    <div>SSID:</div> \
...
void handleRootPost() {
  ...
  if (server.args() == 5
    && server.argName(0).equals("channelName")
    && server.argName(1).equals("token")
    && server.argName(2).equals("ssid")
    && server.argName(3).equals("password")) {
    //it's ok
    
    String data = "[" + server.arg(0) + ":" + server.arg(1) + ":" + server.arg(2) + ":" + server.arg(3) + "]";
    data.replace("+"," ");
  ...
  EEPROM.begin(facade::EEPROM_SIZE);
    ...
    if (status == 1) {
      ...
      if (WiFi.status() != WL_CONNECTED) {
        ...
        mqttClient.init(channelName, token);
      }
...
void sendMeasurements(facade::SensorData measurements) {
  mqttClient.connect();
  Serial.println("Send measurements");

  // BME680
  mqttClient.publish("temperature", measurements.temperature);
  mqttClient.publish("humidity", measurements.humidity);
  mqttClient.publish("pressure", measurements.pressure);
  mqttClient.publish("gasResistance", measurements.gasResistance);
  mqttClient.publish("altitude", measurements.altitude);
  
  // NOVA SDS
  mqttClient.publish("pm10", measurements.pm10);
  mqttClient.publish("pm25", measurements.pm25);

  // SOUND
  mqttClient.publish("noise", measurements.noise);
}
